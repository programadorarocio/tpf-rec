...
if [ ! -f "app/outputs/txt/$filepath" ]; then
...
select opt in "${options[@]}"; do
    case $REPLY in
    1)
        stats_words "app/outputs/txt/$filepath"
        ;;
    2)
        palindrome_detection "app/outputs/txt/$filepath"
        ;;
    3)
        mail_address_detection "app/outputs/txt/$filepath"
        ;;
    4)
        echo "Saliendo..."
        break
